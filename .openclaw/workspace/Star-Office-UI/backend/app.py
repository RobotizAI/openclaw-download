...
def load_state():
    ...
    try:
        ...
        if updated_at and s in WORKING_STATES:
            ...
            if age > ttl:
                state["state"] = "idle"
                state["detail"] = "Aguardando comando (retornando automaticamente à área de descanso)"
                state["progress"] = 0
...
def get_office_name_from_identity():
    """Read office display name from OpenClaw workspace IDENTITY.md (Name field) -> 'Escritório de XXX'."""
    ...
    try:
        ...
        if m:
            ...
            return f"Escritório de {name}" if name else None
    except Exception:
        pass
    return None
...
DEFAULT_AGENTS = [
    {
        ...
        "isMain": True,
        "state": "idle",
        "detail": "Aguardando comando, sempre pronta para te servir.",
        "updated_at": datetime.now().isoformat(),
        "area": "breakroom",
        "source": "local",
        "joinKey": None,
...
def get_yesterday_memo():
    try:
        ...
        if os.path.exists(notas_file):
            with open(notas_file, "r", encoding="utf-8") as nf:
                text = nf.read().strip()
            
            # Formatação em bullet points como as notas originais
            lines = text.split("\n")
            formatted_lines = []
            for line in lines:
                if line.strip():
                    formatted_lines.append("• " + line.strip().lstrip("-").strip())
            memo_content = "\n".join(formatted_lines)
            
            if not memo_content:
                memo_content = "O que quer anotar hoje?"
